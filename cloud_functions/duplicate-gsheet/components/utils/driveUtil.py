...
class Drive:
    ...
    # método crea copia de planilla en carpeta drive
    def set_gsheet_in_folder(self, folder_id, gsheet_id, name_gsheet):
        try:
            # Copiar la hoja de cálculo
            copy_metadata = {
                'name': name_gsheet, 
            }
            copied_sheet = self.get_drive().files().copy(fileId=gsheet_id, body=copy_metadata).execute()

            self.get_drive().files().update(fileId=copied_sheet['id'], addParents=folder_id).execute()
            ...
            self.get_drive().permissions().create(fileId=copied_sheet['id'], body=public_permission).execute()
